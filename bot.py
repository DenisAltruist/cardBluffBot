...
def getHelpRu(message):
    ...
    rules += "На самом деле кроме вскрытия есть возможно блокировать комбинацию (/b). Это значит, что вы говорите, что текущая комбинация - максимальна среди всех возможных в данной раздаче (она должна присутствовать). Если это действительно так, то никто не получит карты в новом раунде. Иначе, карту получает игрок сыгравший /b\n"
